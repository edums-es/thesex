<div class="p-3 w-100">
	...
	{if $sub_view == "coupons"}
		<button data-toggle="modal" data-url="monetization/controller.php?do=add_coupon&node_id={$user->_data['user_id']}&node_type=profile" class="btn btn-sm btn-primary mt-2">
...
<div class="p-3 pt-1">
	{if $sub_view == ""}
		...
		<form class="js_ajax-forms" data-url="users/settings.php?edit=monetization">
			...
			<div class="form-table-row mb-2 pb-1">
				<div>
					<div class="form-label mb-0">{__("Global Discount")}</div>
					<div class="form-text d-none d-sm-block mt-0">{__("Enable or disable global discount for subscriptions plans and paid posts")}</div>
				</div>
				<div class="text-end align-self-center flex-0">
			...
			<div class="form-floating">
				<input type="number" min="1" max="99" class="form-control" name="user_monetization_discount_percent" value="{$user->_data['user_monetization_discount_percent']}" placeholder=" ">
			...
			<div class="row payment-plans">
                <div class="col-md-6 mb-4">
					<div data-toggle="modal" data-url="monetization/controller.php?do=add_plan&node_id={$user->_data['user_id']}&node_type=profile" class="payment-plan new h-100 x_adslist pointer p-3">
					...
					<div class="col-md-6 mb-4">
						<div class="payment-plan h-100 x_adslist p-3">
							...
							<div class="d-flex align-items-center mt-3 gap-2">
								<button type="button" class="btn bg-white flex-1" data-toggle="modal" data-url="monetization/controller.php?do=edit_plan&id={$plan['plan_id']}">
		...
		{if $coupons}
			<div class="table-responsive">
				<table class="table table-hover align-middle js_dataTable">
					<thead>
						<tr>
							<th class="fw-semibold bg-transparent">{__("Code")}</th>
							<th class="fw-semibold bg-transparent">{__("Discount")}</th>
							<th class="fw-semibold bg-transparent">{__("Usage Details")}</th>
							<th class="fw-semibold bg-transparent">{__("One Time")}</th>
							<th class="fw-semibold bg-transparent">{__("Actions")}</th>
						</tr>
					</thead>
					<tbody>
						{foreach $coupons as $coupon}
							<tr>
								<td>
									<span class="badge rounded-pill bg-secondary">{$coupon['coupon_code']}</span>
									<div class="small mt-1">{$coupon['created_date']|date_format:"%e %B %Y"}</div>
								</td>
								<td>
									<span class="badge rounded-pill bg-primary">{$coupon['discount_percent']}%</span>
								</td>
								<td>
									{if $coupon['usage_counter'] > 0}
										<span data-toggle="modal" data-url="monetization/controller.php?do=coupon_users&coupon_id={$coupon['coupon_id']}" class="pointer text-success">
											<span class="badge rounded-pill bg-success">{$coupon['usage_counter']}</span> {__("Users")}
										</span>
									{else}
										<span class="text-muted">{__("No One")}</span>
									{/if}
								</td>
								<td>
								...
								<td>
									<!-- delete -->
									<button class="btn border-0 p-2 rounded-circle lh-1 btn-danger js_monetization-coupon-deleter" data-id="{$coupon['coupon_id']}" title='{__("Delete")}'>
									...
									<!-- delete -->
									<!-- share -->
									<button data-toggle="modal" data-url="monetization/controller.php?do=share_coupon&coupon_id={$coupon['coupon_id']}" class="btn border-0 p-2 rounded-circle lh-1 btn-gray" title='{__("Share")}'>
