{if $_post['for_subscriptions']}
	<div class="main fw-bold small mb-2 d-flex align-items-center gap-1">
		...
		{if $user->_logged_in && $user->_data['user_id'] == $_post['author_id']}
			{__("Creator view: subscribers will see the protected media")}
		{else}
			{__("Subscriptions")}
		{/if}
	</div>
			...
			<div class="post-meta">
				...
				<span class="post-title small">
					...
					{elseif $_post['post_type'] == "video"}
						{__("added video")} {if $_post['video']['category_name']}{__("in")} <a href="{$system['system_url']}/watch/category/{$_post['video']['category_id']}/{$_post['video']['category_url']}">{__($_post['video']['category_name'])}</a>{/if}
				...
				<div class="post-time text-muted">
					...
					{if $_post['manage_post'] && $_post['for_subscriptions']}
						<span class="fw-bold mx-1">·</span>
						{if $_post['subscriptions_image']}
							<span class="badge rounded-pill bg-success bg-opacity-10 text-success border border-success border-opacity-25">
								<i class="fa-solid fa-shield-halved me-1"></i>{__("Protected preview ready")}
							</span>
						{else}
							<span class="badge rounded-pill bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25">
								<i class="fa-solid fa-triangle-exclamation me-1"></i>{__("Exclusive content without preview")}
							</span>
						{/if}
					{/if}
		...
		<div class="flex-0 dropdown">
			...
			<div class="dropdown-menu dropdown-menu-end action-dropdown-menu">
				...
				{if $_post['manage_post']}
					...
					<!-- Monetization -->
					{if $_post['for_subscriptions']}
						<div class="dropdown-item pointer action" data-toggle="modal" data-url="posts/subscription_preview.php?post_id={$_post['post_id']}">
							<i class="fa-solid fa-shield-halved fa-fw"></i>
							<span>{__("Edit exclusive preview")}</span>
						</div>
					{/if}
					<!-- Delete -->
